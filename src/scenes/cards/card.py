...
class Card(pygame.sprite.Sprite):
    ...
    def __init__(self, config: dict):
        ...
        if self.config['type'].lower() == "special":
            self.card_dir = SPECIAL_CARD_DIR
        elif self.config['type'].lower() == "effect":
            self.card_dir = EFFECT_CARD_DIR
        else:
            self.card_dir = PERSONNEL_CARD_DIR

        card_background_path = self.card_dir / CARD_FRONT_BACKGROUND_IMAGE
    ...
    def _draw_item_portrait(self, card_background):
        config = self.config
        pack, image = config['sprite']['sheet'].split('/')
        sheet = pygame.image.load(Path(SPRITE_PACKS[pack], image)).convert_alpha()
        tile_size = self.config['sprite']['sheet_tile_size']
        sprite_position_on_sheet = (config['sprite']['x'], config['sprite']['y'])
        sprite_rect = pygame.Rect(sprite_position_on_sheet[0] * tile_size,
                                  sprite_position_on_sheet[1] * tile_size,
                                  tile_size, tile_size)
    ...
    def _draw_character_portrait(self, card_background):
        ...
        blank_image = pygame.Surface((MAP_TILE_WIDTH // MAP_TILE_SCALE, MAP_TILE_HEIGHT // MAP_TILE_SCALE),
                                     pygame.SRCALPHA)
        sprite_rect = pygame.Rect(portrait_position[0] * MAP_TILE_WIDTH // MAP_TILE_SCALE,
                                  portrait_position[1] * MAP_TILE_HEIGHT // MAP_TILE_SCALE,
                                  MAP_TILE_WIDTH // MAP_TILE_SCALE, MAP_TILE_HEIGHT // MAP_TILE_SCALE)
    ...
    def _draw_title(self, card_background):
        plate_image = pygame.image.load(self.card_dir / CARD_TITLE_IMAGE).convert_alpha()
    ...
    def _draw_type(self, card_background):
        plate_image = pygame.image.load(self.card_dir / CARD_TYPE_IMAGE).convert_alpha()
    ...
    def _draw_text(self, card_background):
        plate_image = pygame.image.load(self.card_dir / CARD_TEXT_IMAGE).convert_alpha()
